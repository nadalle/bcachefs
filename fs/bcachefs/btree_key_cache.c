...
bch2_btree_key_cache_get(struct btree_trans *trans,
		...
		enum six_lock_type lock_type, bool fill)
{
	struct bch_fs *c = trans->c;
	struct bkey_cached *ck;
retry:
	ck = btree_key_cache_find(c, btree_id, pos);
	if (!ck) {
		if (!fill)
			return NULL;

		ck = btree_key_cache_fill(trans, btree_id, pos, lock_type);
		if (!ck)
			goto retry;
		...
		six_lock_type(&ck->lock, lock_type);

		if (ck->key.btree_id != btree_id ||
		    bkey_cmp(ck->key.pos, pos)) {
